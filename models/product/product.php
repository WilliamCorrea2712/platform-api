...
class ProductModel {
    ...
    public function editProductInDatabase($user_id, $product_id, $brand_id, $categories, $price, $cost_price, $weight, $length, $width, $height, $sku, $sort_order, $minimum, $status, $name, $description, $tags, $meta_title, $meta_description, $meta_keyword, $description_resume) {
        ...
        if (!$stmt_product->execute()) {
            $stmt_product->close();
            return createResponse("Erro ao atualizar o produto na tabela: " . $this->conn->error, 500);
        }
        ...
        if (!$stmt_description->execute()) {
            $stmt_description->close();
            return createResponse("Erro ao atualizar a descrição do produto na tabela: " . $this->conn->error, 500);
        }
    ...
    public function deleteProductFromDatabase($user_id, $product_id) {
    ...
    public function saveImageToDatabase($user_id, $product_id, $image_name, $image_url, $alt = null) {

        $get_next_so = "SELECT COALESCE(MAX(sort_order), -1) + 1 AS next_so FROM " . PREFIX . "product_image WHERE product_id = ?";
        $stmt_get_next_so = $this->conn->prepare($get_next_so);
    ...
    public function deleteProductImageFromDatabase($user_id, $product_id, $image_id) {
