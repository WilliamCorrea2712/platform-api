...
function addProductStockToDatabase($product_id, $type_id, $option_ids, $quantity) {
    global $conn;

    try {
        $conn->begin_transaction();

        $sql = "INSERT INTO " . PREFIX . "product_stock (product_id, type_id, option_id, quantity) VALUES (?, ?, ?, ?)";
        ...
        $stmt->bind_param("iiii", $product_id, $type_id, $option_id, $quantity);

        foreach ($option_ids as $option_id) {
            $result = $stmt->execute();
            if (!$result) {
                throw new Exception("Erro ao adicionar estoque do produto.");
            }
        }
        ...
        $stmt->close();
        $conn->close();
        return array("response" => "Estoque do produto adicionado com sucesso.", "status" => 200);
    } catch (Exception $e) {
        $conn->rollback();
        if ($stmt) {
            $stmt->close();
        ...
        $conn->close();
        return array("response" => $e->getMessage(), "status" => 500);
    }
}


function updateProductStock($user_id) {
    if ($_SERVER["REQUEST_METHOD"] == "PUT") {
        $data = json_decode(file_get_contents("php://input"), true);

        if (isset($data['product_id']) && isset($data['option_id']) && isset($data['quantity'])) {
            $product_id = $data['product_id'];
            $option_id = $data['option_id'];
            $quantity = $data['quantity'];

            $result = updateProductStockInDatabase($product_id, $option_id, $quantity);
