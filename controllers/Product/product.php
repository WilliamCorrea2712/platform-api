...
class ProductController {
    public static function addProduct($user_id) {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($data !== null &&
                ...
                $result = $productModel->addProductToDatabaseHelper($user_id, $brand_id, $categories, $price, $cost_price, $weight, $length, $width, $height, $sku, $sort_order, $minimum, $status, $name, $description, $tags, $meta_title, $meta_description, $meta_keyword, $description_resume);
    ...
    public static function getProducts($product_id = null) {
        $productModel = new ProductModel();
        $products = $productModel->getAllProducts($product_id);
    ...
    public static function editProduct($user_id) {
        if ($_SERVER["REQUEST_METHOD"] == "PATCH") { 
            ...
            if (isset($data['product_id'])) {
                ...
                $productModel = new ProductModel();
                $result = $productModel->editProductInDatabase($user_id, $product_id, $brand_id, $categories, $price, $cost_price, $weight, $length, $width, $height, $sku, $sort_order, $minimum, $status, $name, $description, $tags, $meta_title, $meta_description, $meta_keyword, $description_resume);
    ...
    public static function deleteProduct($user_id) {
        if ($_SERVER["REQUEST_METHOD"] == "DELETE") { 
            ...
            if (isset($data['product_id'])) {
                ...
                $productModel = new ProductModel();
                $result = $productModel->deleteProductFromDatabase($user_id, $product_id);
    ...
    public static function addProductImages($user_id) {
        ...
        if ($qtdImage > $max_images) {
            ...
            $productModel = new ProductModel();
            $productModel->saveImageToDatabase($user_id, $product_id, $image_unique_name, $image_url, pathinfo($image_name, PATHINFO_FILENAME));
        }
    ...
    public static function deleteProductImages($user_id) {
        if ($_SERVER["REQUEST_METHOD"] == "DELETE") { 
            $data = json_decode(file_get_contents("php://input"), true);
            ...
            if (isset($data['product_id']) && isset($data['image_id'])) {
                ...
                $productModel = new ProductModel();
                $result = $productModel->deleteProductImageFromDatabase($user_id, $product_id, $image_id);
